#!/bin/sh
# Print various system info to notications
...
if [ $1 = "network" ]; then
	...
	info=$(nmcli dev show)
	while [ $(echo $"$info" | head -n 1 | grep "$dev" | wc -l) -eq 0 ]; do
	...
	if [ $(df -h | grep /storage$ | wc -l) -ne 0 ]; then
	...
	if [ $(df -h | grep $HOME/pCloudDrive$ | wc -l) -ne 0 ]; then
